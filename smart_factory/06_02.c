...
def ps2(): # 센서2 인식 동작
    ...
    if "PS_1=OFF" in serial_receive_data:
    ...
    while True:
        ...
        if ser.in_waiting > 0:
            ...
            if "PS_1=ON" in serial_receive_data:
                ...
                ser.write( sendData.encode() )
                time.sleep(3)
                read_data2 = ser2.readline()
                serial_receive_data2 = read_data2.decode()
                print(serial_receive_data2.split())
                print((serial_receive_data2.split())[0]=='ID=1')
                a=serial_receive_data2.split()
                time.sleep(1)
                if "ID=1" in a:
                    a =""
                    continue
...
for p in ports:
    if 'IOUSBHostDevice' in p.description:
        ser = serial.Serial('/dev/cu.usbmodem1101', 9600)  # 아두이노와 시리얼 통신 연결
        ser2 = serial.Serial('/dev/cu.usbmodem1401', 115200)  # 아두이노와 시리얼 통신 연결
        print(f"포트에 자동으로 연결하였습니다.")
        break
else:
